        ...
        <div className='h-fit w-2/6 bg-white rounded-xl'>
            <div className='p-4 flex flex-col items-center gap-2'>
              <div className='flex justify-start w-full'>
                ...
                <p className='font-bold'>{statusTransfer ? 'Yeay Transfer' : 'Opps Transfer'}<span className={statusTransfer ? 'text-[#1EC15F]' : 'text-[#D00000]'}>{statusTransfer ? 'Succes' : 'Failed'}</span></p>
                <p className='text-[#4F5665] text-xs'>{statusTransfer ? 
                'Thank you for using this application for your financial' 
                : 'Sorry Theres is an issue for your transfer, try again later !'}
                </p>
    
                <button onClick={handleCardStatus} className='p-1 text-sm text-white bg-[#764abc] flex justify-center items-center rounded w-full active:scale-95 transition-all duration-500'>{statusTransfer ? 'Done' : 'Try Again'}</button>
                <Link to={statusTransfer ? '/transfer' : '/dashboard'} className='p-1 text-sm w-full border text-[#764abc] border-[#764abc] flex justify-center items-center rounded active:scale-95 transition-all duration-500'>{statusTransfer ? 'Transfer Again' : 'Back To Dashboard'}</Link>
            </div>
        </div>
    </div>
    )
